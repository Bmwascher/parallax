...
class TestTransportContract:
    ...
    def test_the_codex_binding_regions_are_locked_whole(self):
        ...
        "leading `+` in an exponent. The line check therefore "
        "establishes THREE things and not more: the value is an "
        "object, no comment appears outside a string, and nothing "
        "follows the value but JSON whitespace. Those are the "
        "properties that keep unattributed text out of the record "
        "stream. Full lexical validation is open backlog work, not "
        "a property this check has. **Evidence limit.** This is a "
        "client-echo binding: it proves what the measured Codex "
        "client recorded for this call, never what the server or "
        "model received."
        ) in notes, (
