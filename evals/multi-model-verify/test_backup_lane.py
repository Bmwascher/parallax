...
def test_lane_lock_call_lifecycle_region():
    ...
    assert (
            ...
            "EXPLICITLY thereafter. The owner SHOULD be the harness "
            "session process, not the shell, which exits between calls "
            "and would make every lock instantly stale. `-ResolveOwner` "
            "APPROXIMATES that and does not guarantee it: it walks past "
            "four named TRANSPORTS — `pwsh.exe`, `powershell.exe`, "
            "`cmd.exe`, `conhost.exe` — and returns the FIRST ancestor "
            "outside that set, so under a wrapper whose executable is "
            "named anything else, `node.exe` and `python.exe` among "
            "them, it returns THAT WRAPPER, which exits when its "
            "command does. What is measured is stability across an "
            "added SHELL frame, not under any wrapper; backlog item 26 "
            "is open on exactly the remaining class. A caller that "
            "KNOWS its own session process should pass that identity "
            "instead of resolving one. OTHERWISE, run "
            "`tools/kimi-lane-lock.ps1 -ResolveOwner` once at the start "
            "of the debate, keep its `ownerPid`, `ownerStartTicksUtc` "
            "and `ownerName`, generate one 32-character lowercase "
            "hexadecimal debate id, and hand the pid, the ticks and the "
            "debate id to every later call. Pass `-OwnerName <name>` to "
            "the BUILD and to the LOGIN too: the lock records it, so a "
            "blocked session and the doctor can see WHAT holds the lane "
            "rather than only which pid. It is OPTIONAL everywhere and "
            "is never part of the identity a release must match, "
            "because a caller must not need a display name in order to "
            "let go of its own lock. Build with "
...
def test_doctor_quiet_row_never_becomes_an_expiry():
    ...
    assert (
            ...
            "the files it could open rather than the debate. REPARSE "
            "POINTS ARE NEVER FOLLOWED — not directory junctions, not "
            "symbolic links, not file links — because following one "
            "measures a filesystem object outside the debate home and "
            "can leave the home entirely. A reparse point encountered "
            "anywhere under the home is not skipped either: it makes the "
            "in-home measurement INCOMPLETE, and an incomplete "
            "measurement takes the silence rule above.") in body
