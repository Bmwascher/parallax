# parallax — Codex plugin (NOT a WoW addon)

This repo is developer tooling: a Codex plugin providing cross-model
...
edits are NOT live until you: bump `.Codex-plugin/plugin.json`, run
`Codex plugin marketplace update parallax`, THEN
`Codex plugin update parallax@parallax` (qualified name required),
...
`~/.Codex/plugins/installed_plugins.json`, which records exactly which
