...
with the writer. The release workflow (`.github/workflows/release.yml`)
tags the main head that carries a new version and publishes that section
as the release body; a version with no section, or one the checker
refuses, fails the release rather than shipping it. Items 101, 102 and
103 hold the record.
