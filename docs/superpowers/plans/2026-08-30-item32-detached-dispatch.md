...
So: the two `SKILL.md` calls keep `${CLAUDE_PLUGIN_ROOT}` verbatim - and only those two, because the three in `references/backup-lane.md` carry `<plugin-checkout>` per Global Constraints. Round 22's finding, and the SIXTH site of this shape: the sentence was written in revision 20 when all five calls carried the token, and it sat one paragraph above the blast-radius sentence round 21 corrected and one bullet below the constraint that states the split. and Task 1's outer-command test substitutes the installed plugin path itself, with a comment naming this measurement as the reason a test may do what the skill relies on the harness for.
...
The record below runs to round 22 and the five Fable-lane rounds. Round 19 found this promise outrunning its entries once; round 21 found it again, which is why the entries now end where the debate does rather than where the last edit left them.
...
Round 21 itself found three things. The Task 9 oracle raised `NameError` before it could assert anything, because round 20's own scoping fix had moved an assertion above the line that binds its section - the third oracle in this debate broken by the fix to the previous oracle, and the first defect BOTH LANES found independently in the same round. Three more sites still carried the single-form token claim, in two design bullets and in region one, which the other lane's parallel sweep had not reached. And one pointer named a task below it that sits above it.

Round 22 found the SIXTH single-form site, inside Step 0 itself, and both lanes named that one independently too. It also found this entry recording only the first of round 21's three findings while promising the entries end where the debate does - which is why the entry now reads as it does.

**The Fable lane's third, fourth and fifth rounds** — the scope gap that mattered most in this stretch: the `${CLAUDE_PLUGIN_ROOT}` measurement covers plugin SKILL BODY text, and I had extended it silently to a references file holding three of the five detached calls, where an unsubstituted token expands to EMPTY. The fix needed no new measurement, because `backup-lane.md` already had a pinned placeholder convention. Its fourth round then judged the split as written, rather than as its own recommendation, and named what guards each hazard. Its fifth swept every occurrence of the token, the placeholder and the word anchor, and found a sixth site still claiming one form - inside the very step that records the measurement.
