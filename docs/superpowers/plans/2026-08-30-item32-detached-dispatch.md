...
So: the two `SKILL.md` calls keep `${CLAUDE_PLUGIN_ROOT}` verbatim - and only those two, because the three in `references/backup-lane.md` carry `<plugin-checkout>` per Global Constraints. Round 22's finding, and the SIXTH site of this shape: the sentence was written in revision 20 when all five calls carried the token, and it sat one paragraph above the blast-radius sentence round 21 corrected and one bullet below the constraint that states the split. Task 1's outer-command test substitutes the installed plugin path itself, with a comment naming this measurement as the reason a test may do what the skill relies on the harness for.
...
- `test_a_failure_after_start_kills_the_tree_and_blocks` — inject a failure between start and commit; expect exit 1 and the started process gone. This is the HANDLED case: the `catch` runs, so **no live process tree survives and no receipt is published** - which is the whole guarantee, and not more than it. Round 16's finding: the previous wording said "nothing is left behind", contradicting the transaction itself. By that point the directory is reserved and the wrapper and control files are installed, and the `catch` specifies no filesystem cleanup. Deliberately: a directory left behind is inert and inspectable, while a published receipt or a live child is not. Round 15's finding: it used to claim it was "the state Sol said cannot be eliminated", which is the opposite of what it is. The irreducible case is the separate hard-kill test below, which bypasses the `catch` entirely and lands on `no-receipt`. Keeping the two straight matters, because a handled failure that looks like the irreducible one makes the irreducible one look already covered.
...
Then assert the two REPLACEMENTS arrived, each scoped to its own section, because a negative grep is equally satisfied by a section deleted and by a section rewritten wrongly. Round 15's finding. It is written in python and asserts MEANING and ORDER, not the presence of tokens: round 16 found the first attempt satisfied by "Every committed launch has no receipt; an interrupted launch has a pid", which contains every required token and says the opposite of the contract, and satisfied on order by "liveness is second", which was the very evasion its prose claimed to catch.
...
at = [con.find(n) for n in order]
assert all(i >= 0 for i in at), "missing state names: " + str(
    [n for n, i in zip(order, at) if i < 0])
assert at == sorted(at), "states out of order: " + str(list(zip(order, at)))
...
Sol session `01a055c5-935e-76e3-ad1d-83721bc67d79`.
...
The record below runs to round 23 and the six Fable-lane rounds. Round 19 found this promise outrunning its entries once; the Fable lane's fourth round and round 22 each found it again, which is why the entries now end where the debate does rather than where the last edit left them.
...
**Rounds 20 and 21** — no new completion path in either. Round 20 refused a `-join` claim I had taken from the other lane uncritically, and found four bookkeeping contradictions the previous revision had introduced. Round 21 was dispatched TWICE: the first attempt ended `ERROR: Selected model is at capacity`, exit 1, with no reply written, and was discarded as a transport failure on fresh round-numbered paths - the second discarded attempt in this debate, recorded like round 7's. The round-evidence binder returned CLEAN for it, correctly: it binds the brief this side sent to what the client recorded, and the brief did land. The missing reply and the exit code caught it, not the binder.

Round 21 itself found three things. The Task 9 oracle raised `NameError` before it could assert anything, because round 20's own scoping fix had moved an assertion above the line that binds its section - the third oracle in this debate broken by the fix to the previous oracle, and the first defect BOTH LANES found independently in the same round. FOUR more sites still carried the single-form token claim - two design bullets, region one, and the sentence inside Step 0 itself - which the other lane's parallel sweep had not reached. Only three were repaired; the Step 0 one was missed in the applying and had to be named again in round 22, which is why the entry below credits round 22 with fixing it rather than finding it. And one pointer named a task below it that sits above it, which both lanes had by then noted.

Round 22 closed the SIXTH single-form site, inside Step 0 itself. The cross-vendor lane had already named it in round 21 and named it again when it survived; the Fable lane found it independently in its fifth round. Two lanes, and a site that still took two rounds to land, because the miss was in the applying rather than the finding. It also found this entry recording only the first of round 21's three findings while promising the entries end where the debate does - which is why the entry now reads as it does.

**The Fable lane's third, fourth, fifth and sixth rounds** — the scope gap that mattered most in this stretch: the `${CLAUDE_PLUGIN_ROOT}` measurement covers plugin SKILL BODY text, and I had extended it silently to a references file holding three of the five detached calls, where an unsubstituted token expands to EMPTY. The fix needed no new measurement, because `backup-lane.md` already had a pinned placeholder convention. Its fourth round then judged the split as written, rather than as its own recommendation, and named what guards each hazard. Its fifth swept every occurrence of the token, the placeholder and the word anchor, and found a sixth site still claiming one form - inside the very step that records the measurement. Its sixth found a sentence fragment left by the fix to that site, and refused to carry forward its own "freezable on sight" judgement because the revision had carried more than the two edits that judgement was conditioned on.
...
Round 17 also answered, when asked directly, whether the remaining defect rate belonged to the plan or to re-reading prose: **partly the plan.** The runtime mechanism has been stable since revision 10; the recent defects sit in Task 9's reconciliation text and its oracles, and because Task 9 ships the reconciled spec and closes the backlog items, an oracle that accepts inverted meaning is a real plan defect even though it cannot corrupt the dispatcher. Of the rounds 14 to 16 findings, the stale spec sections and their non-binding oracles could have shipped a materially wrong spec that no then-current oracle would have caught; the handled-failure wording was misleading prose over a correct executable step; and the record's own miscounts ship wrong history and change no behaviour.
