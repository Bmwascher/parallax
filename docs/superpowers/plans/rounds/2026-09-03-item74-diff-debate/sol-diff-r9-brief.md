# Sol diff round 9, brief as sent

The exact text piped to the cross-vendor lane for round 9. Written to disk
before it was sent. Narrow round: two questions.

---

<role>Same adversarial reviewer, same panel, round 9. NARROW round: two questions, not a re-review.</role>

<continuity-check>
Before anything else, state in one line: the branch name, the subject revision you reviewed in round 8, and the terminal verdict you gave it.
</continuity-check>
...
Run `git diff fa86675..20d557a` yourself. Full range `5d20eed..20d557a`.
</subject>

<what-changed>
Record only. Three things, two of them the other lane's round 8
observations, which it offered without making them findings.

R1. Item 69's split summary in the backlog carried the same narrowed
    account the debate record had already corrected: it said the lanes
    split on ownership. They split on ownership AND on status. It now says
    both, and says the narrowed version was put to both lanes in a later
    brief before it was caught.
R2. The void round's cost paragraph counted one dispatch. The remedy was a
    second dispatch, so the true price is two rounds of quota. Stated.
R3. Round 8's briefs and replies are retained, the table and inventory
    carry round 8, and the hash table carries round 8's source hash.
...
- Every dispatched round for the cross-vendor lane bound clean and sealed,
  ...
  later head.
- The same-harness lane's per-round evidence class is dispatch metadata
  only, per `panels.md`, and item 67 says that class cannot detect lost
  continuity at all.
...
round exists: it is a one-field decision on a gating record, and the
session declines to make it alone.
</final-check>
