<role>Adversarial reviewer, equal weight, in a two-model debate (round 3 of the mode-diff debate you opened; round 2 was voided on this side by a transport classification and never read, so this is the first verification you answer).</role>

<task>Verify the application of your round-1 findings on the artifact-roots
range, now cd0e863..HEAD (head 166501e: cf7109b carries every round-1 fix,
166501e corrects the residual statement described in claim 1). The frozen plan and spec are as before; the spec was
...
diff and is appended under <diff> (retained round records excluded).</task>
...
round 1. Budget: this is exchange 3 of 4.
</rules>
...
1. Round-1 claim 4, applied. Every path conversion in
   ...
   asked for. Two residuals stay with `-File` binding on both hosts, exit
   1 without an `ERROR:` line, and are stated in the header and in the
   spec's exit map: a named parameter whose VALUE is missing, and a
   parameter given twice. The session's own probe found the second after
   its earlier wording said "one"; state any THIRD binding fault you can
   reach on either host, or that you found none. Regressions:
...
2. The spec's exit-map paragraph
   ...
   and its reason, and the two residual binding faults, dated as a round-1
...
@@ -83,12 +95,25 @@ function Resolve-Absolute($p) {
     return $full
 }
...
@@ -146,7 +171,7 @@ if (($topExit -ne 0) -or -not $toplevel) {
...
@@ -154,7 +179,7 @@ $top = Normalize-Slashes ([System.IO.Path]::GetFullPath($toplevel))
...
@@ -194,7 +219,14 @@ if ($PSBoundParameters.ContainsKey("DocsRoot")) {
...
@@ -211,7 +243,7 @@ function Resolve-Row($value) {
     $v = $v.TrimEnd("/")
