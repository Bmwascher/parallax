...
- **The clamp**: a contended acquire with `-WaitSeconds 1 -PollSeconds 10` exits 3, leaves the record unchanged, and takes **AT LEAST 0.8 seconds and under five**. The lower bound is the half that catches immediate refusal.
- **Retry SUCCESS**, which no existing test covers: create a real LIVE holder, start a contender with `-WaitSeconds 4 -PollSeconds 1`, assert after one second that the contender HAS NOT EXITED, then release the original holder, and require the contender to acquire before its budget expires, return a new nonce, and leave its own held record. This is what separates a tool that retries from one that sleeps once and gives up.
...
**FIRST USE, when the ENTIRE lane home is absent — one bounded pre-lock exception.** Before anything else, test ONLY whether the configured lane-home DIRECTORY exists. If it is ABSENT: emit the exact shared lane login recovery command, exit nonzero, and **create NOTHING** — no lane home, no lock, no debate home — and do NOT invoke the lock tool at all.
