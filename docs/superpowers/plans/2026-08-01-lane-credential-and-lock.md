...
- **THE LANE LOGIN RECOVERY COMMAND, emitted by EXACTLY TWO SURFACES: Task 6's builder refusal for an absent, unreadable or malformed lane credential, and Task 8's corresponding doctor rows.** It lives here rather than in one task because both emit it and both pin it, and a constant copied into two tasks drifts. **"Every surface that reports a credential status" would have been wider than that**: Task 2's validator prints one exact JSON line, Task 5 reports the same post-run verdicts, and Task 7's preflight promises only a message naming the wrapper, so a universal rule would have contradicted three tasks and left the implementer choosing which text wins.
  ...
  **The command is FAIL-CLOSED, and that is what the semicolons alone would not have given it.** A plain `a; b` chain runs `b` whether or not `a` worked, so an earlier form would have called the login wrapper with an empty owner after a failed resolution. Each invoked script's `$LASTEXITCODE` is checked immediately, and the JSON parse uses `-ErrorAction Stop`, which is the repository's own idiom at `tools/check-drift.ps1:145-149`, `tools/check-drift.ps1:208-210` and `tools/read-kimi-round-evidence.ps1:206-210`.
...
**The probe's fault seam is named: `PARALLAX_LANE_HOME_DIRECTORY_PROBE_FAULT`.** Build mode only; fires immediately BEFORE the real probe; simulates the UNMEASURABLE row; produces no stdout and no recovery command, no mutation, and no lock invocation; and writes one exact stderr sentinel shared by the implementation and the test. Leaving it unnamed meant production and tests had to invent the same variable independently, which is what the other seams are named to prevent.
...
- **Remove mode** uses TERMINATING deletion and then VERIFIES the path is absent before releasing. A deletion error, or a path still present afterwards, is the PRIMARY failure: no `removed` line, NO release, and the held lock record left byte-identical. Its oracle is real rather than seamed — an exclusively opened file beneath the debate home, under both hosts — requiring nonzero, the home still present, no success line, and the lock still held.
- **Failed-build cleanup** keeps the ORIGINAL build failure as primary even when its deletion fails, still ATTEMPTS the release, reports the cleanup error on stderr only, and leaves the lock `free` when that release succeeds. A named cleanup-deletion seam proves both the precedence and the release.
...
- [ ] **Step 1b: The recovery command is EXECUTED, not just compared.** Asserting the emitted string whole is NOT an adequate oracle for a command whose whole job is to run: the earlier broken semicolon form, which called the login wrapper after a failed owner resolution, would have satisfied a string comparison perfectly. So take the string the builder ACTUALLY EMITTED and RUN it, under BOTH hosts.
  ...
  **"Both directions" is not the partition.** The command has THREE dependent boundaries — the owner command's exit, the owner JSON parse, and the login's exit — so a two-row oracle leaves a parse failure and a login failure unexercised. The reachable states are four rows:

  | Owner command | Owner JSON | Login | Required |
  |---|---|---|---|
  | nonzero | none | never invoked | the command exits nonzero |
  | zero | malformed | never invoked | a TERMINATING parse failure |
  | zero | valid | nonzero | login invoked; the command exits nonzero; NO `ok` verdict |
  | zero | valid | zero | the command exits zero; `ok` verdict; a structurally valid fake credential |

  **The escaping needs its own failure direction.** All four rows can use an ordinary path, which would let an implementation that never doubles apostrophes pass every one. So the SUCCESS row uses a resolved lane home CONTAINING AN APOSTROPHE, and requires that the emitted command contains the DOUBLED apostrophe, that it writes only to the intended lane home, and that the credential is structurally `ok`.
