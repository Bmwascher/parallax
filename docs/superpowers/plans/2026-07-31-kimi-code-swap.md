...
- **Exercise the destructive guards against DISPOSABLE targets only.** Each needs a correctly formed sentinel, because a guard that has only ever seen a malformed one has not been tested — but planting an authorizing sentinel on the real user profile or a real drive root and then running newly written recursive-deletion code means that if the guard is defective the test destroys exactly what it exists to protect. So:
  - user-profile branch: run the script in a SUBPROCESS with `USERPROFILE` pointed at a temporary directory, and target that directory. Then run a SECOND case targeting its PARENT, because production refuses the profile "or above it" and equality alone never exercises the ancestor half of that rule.
  - drive-root branch: SELECT an unused drive letter first — enumerate the ones in use and pick one that is not, rather than hard-coding `X:`, which may already be a real or network drive. Run `subst <letter>: <temp dir>`, then VERIFY the mapping resolves to that temp directory before planting anything. If either step fails, skip the branch and say so; do not fall through onto whatever that letter really is, which is the exact accident this redesign exists to prevent. Unmap with `subst <letter>: /d` — the delete form takes the drive argument — in the `finally`.
