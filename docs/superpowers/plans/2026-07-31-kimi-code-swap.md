...
- **Exercise the destructive guards against DISPOSABLE targets only.** Each needs a correctly formed sentinel, because a guard that has only ever seen a malformed one has not been tested — but planting an authorizing sentinel on the real user profile or a real drive root and then running newly written recursive-deletion code means that if the guard is defective the test destroys exactly what it exists to protect. So:
  - user-profile branch: create a DISPOSABLE SANDBOX directory owned by this test, and a fake profile INSIDE it at `<sandbox>/profile`. Run the script in a subprocess with `USERPROFILE` pointed at that child. Target the child for the equality case, then target the SANDBOX for the ancestor case, because production refuses the profile "or above it" and equality alone never exercises the ancestor half. The nesting is load-bearing: if the fake profile were an ordinary child of the system temp directory, the ancestor case would target the shared temp directory itself, which is not disposable and is not this test's to delete.
  - drive-root branch: SELECT an unused drive letter first — enumerate the ones in use and pick one that is not, rather than hard-coding `X:`, which may already be a real or network drive. Run `subst <letter>: <sandbox dir>`, then VERIFY the mapping resolves to that directory before planting anything. **If the letter cannot be selected or the mapping cannot be verified, the live gate FAILS.** It is not a skip: a required safety measurement that cannot be made is never a passing one, which is this plan's own standing invariant, and a skip here would mean the drive-root guard ships unexercised. Never fall through onto whatever that letter really is. Unmap with `subst <letter>: /d` — the delete form takes the drive argument — in the `finally`.
...
- Produces `tools/read-kimi-round-evidence.ps1` with TWO PARAMETER SETS, because a fresh call must DISCOVER the session directory and a resume must be told it. A single signature requiring `-SessionDir` asks the fresh branch for the very thing it exists to establish.

  - **Fresh**: `-Fresh -SessionsRoot <debate-home>/sessions -SessionIdFromStdout <id> -PriorState <json> -Model <id> -Provider <name> -Effort <level> -AgentFile <path> -ExpectedBriefSha256 <hex> [-Json]`. No `-SessionDir`: it is an output, not an input.
  - **Resume**: `-Resume -SessionDir <dir> -PriorState <json> -Model <id> -Provider <name> -Effort <level> -AgentFile <path> -ExpectedBriefSha256 <hex> [-Json]`. No `-SessionsRoot` and no `-SessionIdFromStdout`.

  Passing a fresh-only argument to the resume set, or the reverse, is a parameter-binding error rather than a runtime check.
  ...
  - **Fresh**: `kind: "fresh"` and `knownSessionDirs`, the SESSION-LEAF directories under `<debate-home>/sessions` immediately before dispatch. Normally empty on a new home, but captured rather than assumed.

    **A member is a directory whose name begins `session_`, and nothing else.** The measured topology is `sessions/wd_<workspace>/<session-id>/`, so a debate's FIRST call creates two new directories — the `wd_<workspace>` container and the session inside it. A naive recursive inventory would see two new directories and reject the clean first call of every debate. The `wd_` container is not a session and is never a member.
...
Clean cases:
- fresh round returns `clean` — with an inventory and no offsets, which is the only shape a fresh state has
...
- **a debate's FIRST call, where the client creates both a `wd_<workspace>` container and the session inside it, returns `clean`** — the container is not a session leaf and must not count as a second new directory. Without this the very first call of every debate would fail.
- a `-PriorState` whose `kind` disagrees with the invocation's parameter set fails `state-kind-mismatch`
...
2. `-PriorState.kind` must equal this invocation's parameter set — `fresh` with `-Fresh`, `resume` with `-Resume`. A mismatch means the caller is validating one kind of call against another kind's history: fail `state-kind-mismatch`. Neither the shape check nor the consistency check states this equality, so without it a well-formed resume state could be handed to a fresh invocation.
3. **Establish the session, by kind.**
   - **Fresh**: enumerate the session-leaf directories under `-SessionsRoot` now, by the member definition above, and subtract `-PriorState.knownSessionDirs`. Require EXACTLY ONE new leaf: zero means the call created no session, more than one means something else ran concurrently in this home and the round cannot be attributed. Fail `session-not-resolvable`. Require that leaf's name to equal `-SessionIdFromStdout`; a mismatch means the id the client reported is not the directory being read, so fail `session-id-mismatch`. The resolved leaf becomes the session directory for every rule below. This is how a fresh call gets an identity it could not have had beforehand.
   - **Resume**: `-PriorState.sessionDir` must equal the resolved `-SessionDir` and `-PriorState.sessionId` must equal its name, else fail `state-session-mismatch`. This keeps the exact binding for every call after the first.
4. Internally inconsistent state: a fresh state carrying offsets, hashes or a `sessionDir`; a resume state missing any of them. Fail `state-inconsistent`. Each describes a history that cannot have happened, and a validator that proceeds from one is reasoning from a fiction.
5. `-AgentFile` missing, unreadable, or whose frontmatter does not parse into a name, a `tools` list and a `disallowedTools` list: fail `agent-file-unusable`. The validator compares against it, so an unreadable one makes every comparison vacuous. `-ExpectedBriefSha256` that is not 64 hex characters: fail `bad-argument`.
6. Session directory absent or unreadable: fail `session-dir-missing`. Wire or log file absent: fail `evidence-file-missing`.
7. On a FRESH call the offsets are zero by definition, so rules 8 and 9 are skipped; the whole file is the slice. On a RESUME they apply in full.
8. Either file shorter in BYTES than its prior offset: fail `truncated`. Do NOT re-read from zero — a replacement's opening records may belong to any round while looking like evidence.
...
Because the measurement recorded in `probe-record-2.md` showed `--skills-dir` suppresses nothing observable, this sweep is the PRIMARY control for the reviewed tree's skills and agents, and the text must say so. Describing it as defence in depth would be describing a mitigation as a control.
...
- Dispatch round 1. Take the session id from the client's `To resume this session:` line. Run the validator in its FRESH form: `-Fresh -SessionsRoot <debate-home>/sessions -SessionIdFromStdout <id> -PriorState <the file just written>`. It discovers the session directory itself; do not pass one. Require `status: clean`. **Persist the returned `nextState`** — it is resume-shaped and carries the resolved path and id, both offsets, both prefix hashes and the two continuity hashes. Round 2 cannot be validated without all of it.
- Resume for round 2 with `-m` and `--skills-dir` re-pinned, then run the validator in its RESUME form: `-Resume -SessionDir <the path nextState names> -PriorState <the persisted nextState>`. Require `status: clean`, and confirm it is validating round 2's records and not round 1's.
...
- Retain every artifact under `rounds/2026-07-31-kimi-code-swap/`, then remove the home with `-Remove`.
