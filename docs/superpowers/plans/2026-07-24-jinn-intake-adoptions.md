...
| D3 | Task 1 Step 3's appended sentence hardened; shipped text: "Skills from the user's own codex plugin cache load the same way — record them in the debate record as a non-blocking environment note with the cache path cited, like the global AGENTS.md; not a stop and never a finding." (was: "...note them in the debate record like the global AGENTS.md; not a stop.") | Behavioral gate evidence (three runs of `no-manufactured-objections`: fail/fail/pass): executors phrased the mandated note inconsistently, and the grader's "every noted risk cites a file line and is explicitly non-blocking" expectation graded loose phrasings as under-cited risks. Specifying the note's shape removes the phrasing lottery. Post-D3 confirming run: PASS 3/3 (behavioral-rerun-nmo3.log). | user, 2026-07-24 (gate-fix decision: "Harden the skill sentence") |
| D4 | Final-review fix wave (commit de47bea) added three deltas beyond the frozen task text: (1) `test_missing_rollout_is_named_class` also asserts the literal class name `missing-rollout`; (2) the notes quota probe bullet rewritten from "adoption pending" to adopted form naming doctor check 4b; (3) the check-drift.ps1 env-hygiene comment extended to cover `CODEX_HOME`. | Findings of the final whole-branch review (one Important, two Minors), applied via the SDD single fix wave and verified by its scoped re-review; recorded here because the frozen plan's authority rule makes unrecorded post-freeze changes drift (mode-diff round 1, F6). | user, 2026-07-25 (application-checkpoint authorization, 20260725-de47bea2901b.md) |
