...
| D3 | Task 1 Step 3's appended sentence hardened: the plugin-cache note is now specified as "record them in the debate record as a non-blocking environment note with the cache path cited" (was: "note them in the debate record") | Behavioral gate evidence (three runs of `no-manufactured-objections`: fail/fail/pass): executors phrased the mandated note inconsistently, and the grader's "every noted risk cites a file line and is explicitly non-blocking" expectation graded loose phrasings as under-cited risks. Specifying the note's shape removes the phrasing lottery. | user, 2026-07-24 (gate-fix decision: "Harden the skill sentence") |
