...
| A15 | Five fixes from the user-invoked PANEL: the suppression pass keeps its own instruction report and blocks on a project doc; skills-block PRESENCE is judged on the masked structural text while entries are still read from the raw text; the project-doc delimiter counts only on its own line; `global_agents_md` reports whether the resolved global file exists rather than whether the instructions block rendered; and both of the mirror's git path captures ask git for real pathnames, decoded as UTF-8 on both hosts, with a still-quoted path made an explicit stop. | `tools/codex-context-probe.ps1`, `tools/new-review-mirror.ps1`, both test modules, the design | Mode-diff PANEL (Sol + Kimi + Fable), 2026-07-28, after round 8 had reported no remaining false-clean path. One was exactly that: the second render measured the project doc and threw the answer away, so a `--- project-doc ---` appearing only under the generated override reached status clean and exit 0. Two block too much: a global `AGENTS.md` naming `<skills_instructions>` in prose made the SUPPRESSION proof read a surviving block, and the same file merely mentioning the project-doc delimiter reported the wrong tree — the branch's own test declares the first configuration legitimate but its suppression fixture carried no quote, so no test reached that render. `global_agents_md` was a constant, because Test-PromptShape has already refused to let the block be absent. The git-quoting defect was raised INDEPENDENTLY by two lanes and confirmed live by the driver against a real `café/AGENTS.md`; a third lane then found its dangerous half, where trimming the quotes but not the escapes lets a colliding real path be hashed under the entry the baseline named. Applied under AMENDMENT 8 of the same checkpoint. |

| A16 | Two fixes INSIDE A15's fixes: a complete ordered `<skills_instructions>` pair counts as presence wherever it sits, masking notwithstanding; and the global `AGENTS.md` is resolved with `-LiteralPath` and `-PathType Leaf`, with an existing-but-unresolvable file blocking rather than reporting absent. Two accepted limits added to the design. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design | Mode-diff PANEL round 3, 2026-07-28. A15 had traded a false block for a false CLEAN, which is the direction the brief for that round had named as the thing to attack: `<INSTRUCTIONS>` is masked first, so a skills container NESTED inside it lost its delimiters and its entries with that body and passed suppression. Reproduced on both hosts before fixing. The second is smaller and was also measured on this machine: a DIRECTORY named `AGENTS.md` answered a bare `Test-Path`, and a real `home[1]/AGENTS.md` answered False bare and True literal. Applied under AMENDMENT 9 of the same checkpoint. |

| A17 | The plugin, recommended-plugin and apps markers are judged on masked text, matching what A15 already did for the skills marker; and the dot-source test helpers call `Get-SkillReport` with the same two arguments the script passes. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py` | Mode-diff PANEL round 2, Kimi lane, 2026-07-28. A15's rule was applied only to the one function the finding named, so the identical defect survived one function away: a global `AGENTS.md` whose house rule names `<apps_instructions>` blocked with "the plugin or apps feature is advertising itself". Reproduced against the pre-fix script. The helper change is not a defect - nothing passed vacuously - but the helpers exercised a fallback the script never takes. Applied under AMENDMENT 10 of the same checkpoint. |

| A18 | Presence of every known container family goes through ONE shared rule, `Test-ContainerPresent`: the masked text, or an ordered raw pair recognized by known-name grammar. A paired opener that is not the exact literal is refused as a non-exact shape wherever it sits, nested included, and every paired opener is examined rather than the first. The global-file measurement is wholly inside one guard with every cmdlet stopping on error. The dot-source test helpers pass the script body through a file. | `tools/codex-context-probe.ps1`, both test modules | Mode-diff PANEL round 4, 2026-07-28. A17's fix was applied to the one function that had been named, so the identical false clean survived one function away: `<INSTRUCTIONS><apps_instructions>live</apps_instructions></INSTRUCTIONS>` reported no apps feature. A16's raw-pair backstop matched an exact literal only, so a nested `<skills_instructions version="2">` pair stayed invisible. All three defects, plus a `CODEX_HOME` naming a missing provider reaching a clean report that said the user has no global `AGENTS.md`, were reproduced on both hosts before fixing. The shared rule is the point: three consecutive rounds each fixed one family and left the next one open. Applied under AMENDMENT 11 of the same checkpoint. |

| A19 | The shared rule matches a complete SURFACE rather than an exact pair: a self-closing known tag counts, both delimiters are matched as grammar, and a surface whose literals are not exact is refused. The global-file location fallback moves inside the guard that claimed to cover it. The design's accepted limit is broadened from skills to all four families and to cross-body pairing. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design | Mode-diff PANEL round 5, 2026-07-28. A18's backstop matched the opener by grammar and the close by exact literal, so from inside a masked body both `<apps_instructions/>` and `<apps_instructions>live</apps_instructions >` were no surface at all and reached a clean report. Reproduced on both hosts before fixing. The fallback `Join-Path` sat outside the guard whose own comment said every cmdlet in the measurement was guarded; no false clean followed from it, and the description was still wrong. Applied under AMENDMENT 12 of the same checkpoint. |

| A20 | The known-family tag grammar gets a real name boundary and quote-aware attributes: only whitespace, `/` or `>` may follow the name, a quoted attribute value may contain `>`, and the self-closing slash is captured at the tag's real terminator. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py` | Mode-diff PANEL round 6, 2026-07-28. One missing rule produced both directions at once: `<apps_instructions note="a>b"/>` truncated at the quoted `>`, was discarded as unpaired, and reached a clean report; while the truncated form of `<apps_instructions note="literal/> only">` ended in `/>` and blocked a legitimate house rule. Separately, `\b` ends at `-`, `.` and `:`, all legal in a tag NAME here, so `<apps_instructions-extra/>` - a different tag - was rejected as a malformed apps tag. All reproduced on both hosts. Applied under AMENDMENT 13 of the same checkpoint. |

| A21 | **A DESIGN CHANGE, not a fix.** The suppression check stops asking whether a known family's markup is a real container. A family name occurring at all, case-insensitively, anywhere in a render, stops the run. `Get-RawContainerSurface`, `Test-ContainerPresent` and `Get-StructuralText` are deleted; `Get-SkillReport` and `Get-FeatureReport` take one argument again. 22 tests changed, of which 6 test functions - 8 collected cases - REVERSED from expecting a pass to expecting a block. An earlier version of this row said nine, which was wrong and a reviewer counted it. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design | Not a panel finding: the user chose it, on my recommendation, after six panel rounds each found a new shape that slipped past the previous round's structural rule, five of them reaching a CLEAN report. Flattened text does not distinguish a real block from a quoted one, so the parser was being asked a question its input cannot answer. The blunt rule cannot produce a false clean and can only produce a false block, which is loud and self-diagnosing. The cost is real and documented in the design's source list, which this row described too narrowly in two ways and is corrected here: a mention of one of the three FEATURE families stops reviews wherever it is rendered, while a mention of the SKILLS family in a skill description costs nothing, and several sources of a mention cannot be reworded at all. The first render keeps the full parser, because enumerating and classifying skills is where precision earns its keep. Applied under AMENDMENT 14 of the same checkpoint. |

| A22 | Both lanes reviewed the design change and both endorsed the trade; neither found a false-clean path in the suppression proof. Their findings: line breaks are removed before the family-name search, because the parser's own chunk join destroyed a name split across a boundary; the FIRST render requires the skills container in its exact form and reads entries only from inside it, because prose could otherwise supply fake entries into the generated override; the suppression render is tested directly and never parsed; the two test helpers stop passing an inert argument (raised independently by both lanes); and the recorded cost, the skills-versus-feature asymmetry, the reversal count and one stale sentence are all corrected. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design, this file | Mode-diff PANEL round 7 (Sol) and round 3 (Kimi), 2026-07-28. The serious one is the first render: reproduced before fixing, with the renderer emitting no skills block, prose in the user's own AGENTS.md supplied a heading and an entry line and the probe reported `status: clean` with `skills_before: 1`, writing an override built from a skill that was never advertised. A shape-change detector cannot assume the shape it exists to verify. Applied under AMENDMENT 15 of the same checkpoint. |

| A23 | The first render blanks every OTHER known container's body, space for character, before it looks for the skills container, and refuses the measurement unless exactly one opener and one close survive in that order. Two record gaps closed: a skill description naming a FEATURE family, and a family name spelled across a line break. Three stale claims that every false block is cleared by rewording one line are corrected, and the stale motivation on `Get-SkillReport` is re-pointed at its two real callers. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design, this file | Mode-diff PANEL round 8 (Sol) and round 4 (Kimi), 2026-07-28. Sol found A22's fix incomplete one function away, in the same class it was meant to close: the first render searched RAW text while the shape scanner blanked known bodies, and the render puts `<permissions instructions>` first. Reproduced on both hosts, and end to end: 29 real entries became one fake entry, `status: clean`, `skills_before: 1`, and an override that disabled the fake and left every real skill loaded. Reaching that report needs the quoted text to be absent from the SECOND render, which on today's client it never is - so the shipped flow stopped on the blunt rule instead, one rule away, for a reason unrelated to the defect. Kimi's independent round returned PASS on the gate and named the two record gaps. Applied under AMENDMENT 16 of the same checkpoint. |

| A24 | The blanked text LOCATES the skills container and nothing else; the entries are read from the raw render at those offsets. Ambiguity is judged on every shape, not only shapes carrying an opener. The ambiguity message stops claiming a completed blanking, and `Get-SkillReport`'s comment stops claiming two callers. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design, this file | Mode-diff PANEL round 9 (Sol) and round 5 (Kimi), 2026-07-28. A23's own fix carried the next defect, the fourth time in this cycle. Blanking was added to locate the container, and the entries were then read out of the blanked text as well: with the genuine permissions container absent from the render, a skill description quoting that pair around a later entry blanked the entry with it, and the report returned 28 of 29 skills with `Malformed` false, `Ambiguous` false, and nothing to show one had gone missing. Reproduced on both hosts. Kimi returned PASS with no findings on the same head and read the same line as correct, which is why the split was resolved by running the case. Applied under AMENDMENT 17 of the same checkpoint. |

| A25 | The locator blanks the same container list in the same order as `Test-PromptShape`, skills family included, so the two functions cannot hold two answers about one text. Locating is unaffected: only bodies are blanked and the delimiters survive, and a second skills container nested in the body is still refused, because the count rule runs before that body is blanked. A blanking failure's own message travels with the verdict and the caller names it. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, this file | Mode-diff PANEL round 10 (Sol, PASS) and round 6 (Kimi, FIX). The lanes split in the other direction this round: Sol attacked the offset invariant with CRLF, sequential containers and non-BMP characters, found no drift, and said the design is sound now that locating, reading and proving are separated; Kimi found the loop still skipping one family. Reproduced on both hosts, which that lane could not do: the shape scan PASSED while `Get-SkillReport` called the SKILLS boundaries ambiguous at one opener and one close, refusing a legitimate render and naming the wrong container. Applied under AMENDMENT 18 of the same checkpoint. |

| A26 | The ambiguity reason is built by `Get-AmbiguityReason`: when the blanking pass failed it reports that no span was established and names the real cause, and otherwise it reports the counts. Extracting it is what makes the wording testable, since the branch sits behind `Test-PromptShape` and no run reaches it. Two records that still said every OTHER known container's body is blanked are corrected. | `tools/codex-context-probe.ps1`, `evals/multi-model-verify/test_codex_context_probe.py`, the design, this file | Mode-diff PANEL round 11 (Sol) and round 7 (Kimi), 2026-07-28, both FIX and both Minor. Sol opened by withdrawing its own round-10 PASS: the other lane's finding was valid and it had missed the list mismatch. Its own finding was the caller's message, which named the SKILLS boundaries ambiguous and printed valid skills counts before appending another container's failure - a contradiction inside one message, and the sole reason it would not sign. Kimi found the stale word in two records. **The round-7 brief did not reach the Kimi lane**: its reply records the text as truncated after "New head:", follows the standing round-1 structure rather than this round's five tasks, and never answers the sign-off question. The route evidence was sound; the prompt was not. Applied under AMENDMENT 19 of the same checkpoint. |
