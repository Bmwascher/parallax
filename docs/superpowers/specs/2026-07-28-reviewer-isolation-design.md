...
| a known block appears in any form but its exact opening literal, anywhere on a line, INCLUDING a case variant, OUTSIDE any unambiguously masked container body | transport failure | blocked; every dedicated parser matches an exact literal, so any other form is invisible to all of them. Name recognition is case-insensitive and the literal allowlist is case-sensitive, deliberately: with both case-sensitive, an uppercase variant was skipped as a known name by PowerShell's case-insensitive `-contains`. The scope qualifier is load-bearing: a malformed known tag QUOTED inside a validated body - a user's own AGENTS.md, or a skill description, both free text - is deliberately ignored, because blocking it rejected legitimate input (rounds 6 and 7) |
